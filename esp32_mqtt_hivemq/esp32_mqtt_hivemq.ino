...
#define ssid "mixao"
#define password "888888888"
//#define mqtt_server "broker.mqtt-dashboard.com"
#define mqtt_server "192.168.43.41"

const uint16_t mqtt_port = 1883; // Port của MQTT

#define topic1 "temperature-humidity116k"
...
void setup()
{
  ...
  //      reconnect();
  //    }
  client.subscribe("livingroomLight116k");
  client.subscribe("livingroomAirConditioner116k");
  client.subscribe("television116k");
  client.subscribe("bedroomLight116k");
  client.subscribe("bedroomAirConditioner116k");
  client.subscribe("airVent116k");

  pinMode(D1, OUTPUT); // livingroomLight116
  pinMode(D2, OUTPUT); // livingroomAirConditioner116
...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  if (String(topic) == "livingroomLight116k")
  {
  ...
  if (String(topic) == "livingroomAirConditioner116k")
  {
  ...
  if (String(topic) == "television116k")
  {
  ...
  if (String(topic) == "bedroomLight116k")
  {
  ...
  if (String(topic) == "bedroomAirConditioner116k")
...
void loop()
{
  ...
  if (now - lastMsg > 2000)
  {
    ...
    client.publish("temperature-humidity116k", buffer);
